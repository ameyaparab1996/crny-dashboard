...
# PLOT BAR CHART:
community_bar_fig = px.bar(
    community_df,
    x='community',
    y='Number of Artists',
    color='community',
    color_discrete_sequence= [colors.to_hex(plt.cm.get_cmap('plasma')(i)) for i in np.linspace(0.2, 0.8, len(community_df['community'].unique()))],
    #width=550,
    #height=500,
    hover_name='community',
    hover_data={'community': False, 'Number of Artists': True}
)

community_bar_fig.update_layout(
    ...
    xaxis=dict(
        ...
        zeroline=False,
        title=dict(
            text='<i>Community wise distribution</i>',
            font=dict(size=14),
            standoff=40
        )
    )
...
community_bar_fig.update_traces(
